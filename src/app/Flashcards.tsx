...
export default function FlashcardsPage() {
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-black tracking-[-0.04em]">Flashcards</h1><p className="text-sm text-[#2a1e16]/68">Study and retain the key concepts — tap a card to flip it</p></div>

      <div className="glass-card rounded-2xl p-4">
        <div className="flex flex-wrap gap-2">
          {DECKS.map((d) => (
            <button key={d} type="button" onClick={() => pickDeck(d)} className={`rounded-full px-4 py-1.5 text-xs font-bold transition ${deck === d ? "bg-orange-500 text-white" : "border border-[#2a1e16]/12 bg-[#2a1e16]/5 text-[#2a1e16]/68 hover:text-[#2a1e16]"}`}>{d}</button>
          ))}
        </div>
      </div>
      ...
      <button type="button" onClick={() => setFlipped((f) => !f)} className="relative block min-h-[16rem] w-full">
        <div className={`glass-card grid min-h-[16rem] place-items-center rounded-3xl p-8 text-center transition-all duration-300 ${flipped ? "border-[#ea580c]/30" : "border-orange-300/20"}`} style={{ background: flipped ? "radial-gradient(ellipse at 50% 0%, rgba(234,88,12,0.14) 0%, transparent 60%)" : "radial-gradient(ellipse at 50% 0%, rgba(249,115,22,0.14) 0%, transparent 60%)" }}>
          <div>
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-[#2a1e16]/45">{flipped ? "Answer" : `${card.deck} · Question`}</p>
            <p className={`mt-4 font-black leading-snug ${flipped ? "text-lg text-[#2a1e16]/88" : "text-2xl"}`}>{flipped ? card.back : card.front}</p>
