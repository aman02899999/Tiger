...
function DoshaQuiz() {
  ...
  return (
    <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6 md:p-8">
      <div className="mb-6 flex items-center gap-3">
        <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-violet-500/20 text-xl">
          🌿
        </div>
        <div>
          <h2 className="text-xl font-bold text-[#e9f3f5]">Prakriti (Dosha) Quiz</h2>
          <p className="text-sm text-[#e9f3f5]/60">Discover your Ayurvedic constitution</p>
        </div>
      </div>
      ...
      {!result && (
        <div className="space-y-6">
          {doshaQuestions.map((q, qIdx) => (
            <div key={qIdx} className="rounded-xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-4">
              <p className="mb-3 font-medium text-[#e9f3f5]">
                {qIdx + 1}. {q.question}
              </p>
                  ...
                  <button
                    ...
                    className={`w-full rounded-lg border px-4 py-2.5 text-left text-sm transition-all ${
                      answers[qIdx] === opt.dosha
                        ? "border-violet-500 bg-violet-500/20 text-[#e9f3f5]"
                        : "border-[#e9f3f5]/10 bg-transparent text-[#e9f3f5]/70 hover:border-[#e9f3f5]/30 hover:text-[#e9f3f5]"
                    }`}
                  >
                    {opt.label}
                  </button>
          ...
          {!allAnswered && (
            <p className="text-center text-sm text-[#e9f3f5]/40">
      ...
      {result && info && (
        <div className="space-y-5">
          ...
          >
            <p className="mb-1 text-sm uppercase tracking-widest text-[#e9f3f5]/60">Your Dominant Dosha</p>
            <h3 className="mb-2 text-2xl sm:text-3xl font-extrabold capitalize" style={{ color: info.color }}>
              {result}
            </h3>
            <p className="text-sm leading-relaxed text-[#e9f3f5]/80">{info.desc}</p>
          </div>

          <div className="rounded-xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-4">
            <h4 className="mb-3 font-semibold text-[#e9f3f5]">Diet Recommendations</h4>
            <ul className="space-y-2">
              {info.diet.map((tip, i) => (
                <li key={i} className="flex items-start gap-2 text-sm text-[#e9f3f5]/70">
          ...
          <button
            ...
            className="w-full rounded-xl border border-[#e9f3f5]/20 py-2.5 text-sm text-[#e9f3f5]/70 transition hover:text-[#e9f3f5]"
          >
            Retake Quiz
          </button>
        </div>
...
function ConditionCard({ condition, onClick }: { condition: typeof conditions[0]; onClick: () => void }) {
  return (
    <button
      onClick={onClick}
      className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-5 text-left transition-all hover:border-violet-500/40 hover:bg-violet-500/10"
    >
      <div className="mb-3 text-3xl">{condition.icon}</div>
      <h3 className="font-semibold text-[#e9f3f5]">{condition.name}</h3>
      <p className="mt-0.5 text-xs text-[#ffb627]">{condition.sanskrit}</p>
      <p className="mt-2 text-xs text-[#e9f3f5]/50">{condition.herbs.slice(0, 2).join(", ")} +{condition.herbs.length - 2} herbs</p>
    </button>
  );
}
    ...
    <div className="rounded-2xl border border-violet-500/30 bg-violet-500/10 p-6">
      <div className="mb-5 flex items-start justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h3 className="text-xl font-bold text-[#e9f3f5]">{condition.name}</h3>
            <p className="text-sm text-[#ffb627]">{condition.sanskrit}</p>
          </div>
        </div>
        <button
          onClick={onClose}
          className="rounded-lg border border-[#e9f3f5]/10 px-3 py-1 text-sm text-[#e9f3f5]/50 hover:text-[#e9f3f5]"
        >
          ✕ Close
        </button>
      </div>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        <div className="rounded-xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-4">
          ...
          <div className="flex flex-wrap gap-2">
            {condition.herbs.map((h) => (
              <span key={h} className="rounded-full bg-violet-500/20 px-3 py-1 text-xs text-[#e9f3f5]">
                {h}
              </span>
            ))}
          </div>
        ...
        <div className="rounded-xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-4">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-[#ffb627]">Formulation</h4>
          <p className="text-sm font-medium text-[#e9f3f5]">{condition.formulation}</p>
          <p className="mt-1 text-xs text-[#e9f3f5]/50">Consult an Ayurvedic physician before use</p>
        </div>

        <div className="rounded-xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-4">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-fuchsia-400">Diet Protocol</h4>
          <p className="text-sm leading-relaxed text-[#e9f3f5]/80">{condition.diet}</p>
        </div>

        <div className="rounded-xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-4">
          <h4 className="mb-2 text-xs font-semibold uppercase tracking-wider text-emerald-400">Yoga & Practices</h4>
          <p className="text-sm leading-relaxed text-[#e9f3f5]/80">{condition.yoga}</p>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6 md:p-8">
      <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h2 className="text-xl font-bold text-[#e9f3f5]">Herb Encyclopedia</h2>
          <p className="text-sm text-[#e9f3f5]/60">10 key Ayurvedic herbs and their applications</p>
        </div>
        <input
          type="text"
          ...
          className="rounded-xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 px-4 py-2 text-sm text-[#e9f3f5] placeholder-[#e9f3f5]/30 outline-none focus:border-violet-500/50 sm:w-56"
        />
      </div>
      ...
      <div className="overflow-x-auto">
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b border-[#e9f3f5]/10">
              <th className="pb-3 text-left font-semibold text-[#ffb627]">Herb</th>
              <th className="pb-3 text-left font-semibold text-[#ffb627]">Sanskrit / Botanical</th>
              <th className="pb-3 text-left font-semibold text-[#ffb627]">Key Benefits</th>
              <th className="pb-3 text-left font-semibold text-[#ffb627]">Dosage</th>
              <th className="pb-3 text-left font-semibold text-[#ffb627]">Caution</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-[#e9f3f5]/5">
            {filtered.map((herb) => (
              <tr key={herb.name} className="transition-colors hover:bg-[#e9f3f5]/5">
                <td className="py-3 pr-4 font-medium text-[#e9f3f5]">{herb.name}</td>
                <td className="py-3 pr-4 text-[#e9f3f5]/50 italic">{herb.sanskrit}</td>
                <td className="py-3 pr-4 text-[#e9f3f5]/70 max-w-[200px]">{herb.benefits}</td>
                <td className="py-3 pr-4 text-[#e9f3f5]/70 max-w-[160px]">{herb.dosage}</td>
                <td className="py-3 text-amber-400/80 max-w-[160px]">{herb.caution}</td>
              </tr>
            ))}
            {filtered.length === 0 && (
              <tr>
                <td colSpan={5} className="py-8 text-center text-[#e9f3f5]/30">
    ...
    <div className="space-y-8">
      <div>
        <div className="mb-2 flex items-center gap-2">
          ...
          <span className="rounded-full bg-[#ffb627]/20 px-3 py-1 text-xs font-medium text-[#ffb627]">
            Modern Application
          </span>
        </div>
        <h1 className="text-xl sm:text-3xl md:text-4xl font-extrabold text-[#e9f3f5]">
          Ayurveda Hub
        </h1>
        <p className="mt-2 max-w-2xl text-[#e9f3f5]/60">
      ...
      <div>
        <h2 className="mb-4 text-xl font-bold text-[#e9f3f5]">Conditions & Protocols</h2>
        <p className="mb-5 text-sm text-[#e9f3f5]/60">
