  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/68">Export everything you've tracked, or restore it on another device</p>
      </div>

      <div className="grid gap-6 lg:grid-cols-2">
        {/* Export / import */}
        <div className="glass-card rounded-2xl p-6">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-600">💾 Your data</p>
          <div className="mt-4 flex gap-3">
            <div className="flex-1 rounded-xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-4 text-center">
              <p className="text-3xl font-black tabular-nums text-[#ea580c]">{keyCount}</p>
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#2a1e16]/65">saved items</p>
            </div>
            <div className="flex-1 rounded-xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-4 text-center">
              <p className="text-3xl font-black tabular-nums text-orange-600">{sizeKb}</p>
              <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#2a1e16]/65">KB total</p>
            </div>
          </div>

          <button type="button" onClick={exportData} className="btn-gloss mt-4 w-full rounded-full bg-gradient-to-r from-orange-300 via-amber-500 to-orange-700 py-3 text-xs font-black uppercase tracking-[0.16em] text-white">⬇ Export Backup (JSON)</button>
          <button type="button" onClick={() => fileRef.current?.click()} className="mt-2 w-full rounded-full border border-[#2a1e16]/15 bg-[#2a1e16]/5 py-3 text-xs font-black uppercase tracking-[0.16em] text-[#2a1e16]/80 hover:bg-[#2a1e16]/10">⬆ Import Backup</button>
        ...
        {/* App tour */}
        <div className="glass-card rounded-2xl p-6">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#ea580c]">🗺️ Quick tour</p>
          <h2 className="mt-1 text-lg font-black">Everything The Titan Fitness offers</h2>
          <div className="mt-4 space-y-2.5">
            {TOUR.map((t) => (
              <div key={t.title} className="flex items-start gap-3 rounded-xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-3">
                <span className="text-2xl">{t.icon}</span>
                <div>
                  <p className="text-sm font-bold">{t.title}</p>
                  <p className="text-xs text-[#2a1e16]/68">{t.desc}</p>
                </div>
              </div>
            ))}
          </div>
