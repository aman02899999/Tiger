...
const RESULTS: Record<Dosha, { name: string; icon: string; color: string; summary: string; tips: string[] }> = {
  ...
  kapha: { name: "Kapha", icon: "🌱", color: "#34d399", summary: "Earth & water — you're calm, strong, and steady, but can tip into weight gain, congestion, and lethargy when out of balance.", tips: ["Favor light, warm, spiced foods; avoid heavy, oily, sweet foods", "Stay active with vigorous, energizing exercise", "Rise early and avoid daytime naps", "Embrace variety and new stimulation to stay motivated"] },
};
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-sm text-[#f7f0df]/68">Based on your answers — your Ayurvedic constitution (Prakriti)</p>
        </div>
        ...
        <div className="glass-card rounded-3xl p-8 text-center" style={{ background: `radial-gradient(ellipse at 50% 0%, ${r.color}22 0%, transparent 60%)` }}>
          ...
          <p className="mx-auto mt-3 max-w-lg text-sm leading-relaxed text-[#f7f0df]/78">{r.summary}</p>
        </div>

        {/* Breakdown bars */}
        <div className="glass-card rounded-2xl p-6">
          <p className="mb-4 text-xs font-bold uppercase tracking-[0.2em] text-violet-300">Your balance</p>
          <div className="space-y-3">
            {(Object.keys(RESULTS) as Dosha[]).map((d) => (
              <div key={d} className="flex items-center gap-3">
                ...
                <div className="h-2.5 flex-1 overflow-hidden rounded-full bg-[#f7f0df]/10">
        ...
        {/* Recommendations */}
        <div className="glass-card rounded-2xl p-6">
          <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-[#d8b35a]">How to stay in balance</p>
          <ul className="space-y-2">
            {r.tips.map((t, i) => <li key={i} className="flex gap-2 text-sm text-[#f7f0df]/80"><span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full" style={{ background: r.color }} />{t}</li>)}
          </ul>
        </div>

        <div className="flex flex-wrap gap-3">
          <button type="button" onClick={restart} className="btn-gloss rounded-full bg-gradient-to-r from-violet-300 via-fuchsia-500 to-violet-700 px-6 py-3 text-xs font-black uppercase tracking-[0.16em] text-white">Retake Quiz</button>
        </div>
        <p className="text-center text-[11px] text-[#f7f0df]/55">Explore the Ayurveda Library for herbs, routines, and remedies suited to your dosha. Educational only — not medical advice.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#f7f0df]/68">Discover your Ayurvedic mind-body type in {QUESTIONS.length} questions</p>
      </div>

      <div className="h-2 w-full overflow-hidden rounded-full bg-[#f7f0df]/10">
        <div className="h-full rounded-full bg-gradient-to-r from-violet-400 to-fuchsia-400 transition-all duration-300" style={{ width: `${progress}%` }} />
      </div>
      ...
      <div className="glass-card rounded-2xl p-6">
        ...
        <div className="mt-5 space-y-3">
          {q.options.map((o, i) => (
            <button key={i} type="button" onClick={() => choose(o.dosha)} className="block w-full rounded-xl border border-[#f7f0df]/12 bg-[#f7f0df]/5 p-4 text-left text-sm font-semibold transition hover:border-violet-200/40 hover:bg-violet-400/10">
              {o.label}
            </button>
          ))}
        </div>
      ...
      {step > 0 && (
        <button type="button" onClick={() => { setStep(step - 1); setAnswers(answers.slice(0, -1)); }} className="text-sm font-bold text-[#f7f0df]/60 hover:text-[#f7f0df]">← Previous question</button>
      )}
    </div>
  );
}
