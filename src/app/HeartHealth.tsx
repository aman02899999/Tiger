...
const TECHNIQUES: Technique[] = [
  { id: "resonance", name: "Heart Coherence", tagline: "5.5 breaths / min", benefit: "The clinically-studied pace for heart-rate variability and calm focus.", inhale: 5.5, hold: 0, exhale: 5.5, holdOut: 0, accent: "#e879f9" },
  { id: "box", name: "Box Breathing", tagline: "4 · 4 · 4 · 4", benefit: "Navy-SEAL technique to steady nerves and lower stress fast.", inhale: 4, hold: 4, exhale: 4, holdOut: 4, accent: "#a78bfa" },
...
export default function HeartHealthPage() {
  ...
  const cohColor = coherence >= 75 ? "#34d399" : coherence >= 45 ? "#d8b35a" : "#e879f9";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#f7f0df]/68">Guided coherence breathing to calm your nervous system and strengthen your heart</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {TECHNIQUES.map((t) => (
          <button
            ...
            type="button"
            onClick={() => setTech(t)}
            className={`rounded-2xl border p-4 text-left transition ${tech.id === t.id ? "border-white/25 bg-white/10" : "border-[#f7f0df]/10 bg-[#f7f0df]/5 hover:bg-[#f7f0df]/10"}`}
          ...
          >
            ...
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#f7f0df]/62">{t.tagline}</p>
            <p className="mt-2 text-xs leading-relaxed text-[#f7f0df]/68">{t.benefit}</p>
          </button>
        ))}
      </div>
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div className="relative grid h-72 w-72 place-items-center">
            ...
            <div
              ...
              style={{
                background: `radial-gradient(circle at 35% 30%, ${tech.accent}, #4c1d95 70%)`,
          ...
          <div className="mt-6 flex items-center gap-3">
            <button
              ...
              style={{ background: `linear-gradient(90deg, ${tech.accent}, #7c3aed)` }}
            >
            ...
            <button
              ...
              className="rounded-full border border-[#f7f0df]/15 px-6 py-4 text-xs font-black uppercase tracking-[0.16em] text-[#f7f0df]/70 hover:bg-[#f7f0df]/8"
            >
              Reset
            </button>
          </div>
          <p className="mt-3 text-xs text-[#f7f0df]/62">Follow the orb — breathe in as it grows, out as it shrinks. {tech.name} · {tech.tagline}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="glass-card rounded-2xl p-6 text-center">
          ...
          <p className="mt-3 text-3xl font-black tabular-nums">{bpm}<span className="text-sm font-normal text-[#f7f0df]/62"> bpm</span></p>
          <p className="text-[11px] text-[#f7f0df]/62">{running ? "settling as you breathe…" : "start to see it drop"}</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 text-center">
          ...
          <div className="relative mx-auto mt-3 h-24 w-24">
            ...
            <div className="absolute inset-[7px] grid place-items-center rounded-full bg-[#0b0714]">
              <span className="text-2xl font-black tabular-nums" style={{ color: cohColor }}>{Math.round(coherence)}</span>
            </div>
          </div>
          <p className="mt-3 text-[11px] text-[#f7f0df]/62">{coherence >= 75 ? "In the coherence zone! 🌊" : "keep the rhythm steady"}</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 text-center">
          ...
          <p className="text-[11px] text-[#f7f0df]/62">{cycles} full breaths</p>
          ...
          <p className="mt-1.5 text-[10px] text-[#f7f0df]/55">{elapsed >= 120 ? "Goal reached · +20 XP 🎉" : "Breathe 2 min for +20 XP"}</p>
        </div>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            ...
            <div key={s.t} className="rounded-xl border border-white/8 bg-white/5 p-4">
              ...
              <p className="mt-2 text-sm font-bold text-[#f7f0df]">{s.t}</p>
              <p className="mt-1 text-xs leading-relaxed text-[#f7f0df]/68">{s.d}</p>
            </div>
          ))}
        </div>
        <p className="mt-4 text-[11px] text-[#f7f0df]/55">Heart rate and coherence shown here are illustrative guides to keep you in rhythm, not medical measurements.</p>
      </div>
    </div>
  );
}
