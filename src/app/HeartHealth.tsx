...
const TECHNIQUES: Technique[] = [
  { id: "resonance", name: "Heart Coherence", tagline: "5.5 breaths / min", benefit: "The clinically-studied pace for heart-rate variability and calm focus.", inhale: 5.5, hold: 0, exhale: 5.5, holdOut: 0, accent: "#3b9dff" },
  { id: "box", name: "Box Breathing", tagline: "4 · 4 · 4 · 4", benefit: "Navy-SEAL technique to steady nerves and lower stress fast.", inhale: 4, hold: 4, exhale: 4, holdOut: 4, accent: "#2dd4bf" },
  { id: "478", name: "4-7-8 Relax", tagline: "Sleep & anxiety", benefit: "A longer exhale flips on the parasympathetic 'rest & digest' system.", inhale: 4, hold: 7, exhale: 8, holdOut: 0, accent: "#60b6fa" },
  { id: "calm", name: "Calm Down", tagline: "4 · 6 extended exhale", benefit: "Quick reset — longer out-breath drops your heart rate within a minute.", inhale: 4, hold: 0, exhale: 6, holdOut: 0, accent: "#34e08a" },
];
...
export default function HeartHealthPage() {
  ...
  const cohColor = coherence >= 75 ? "#34e08a" : coherence >= 45 ? "#ffb627" : "#3b9dff";
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#e9f3f5]/68">Guided coherence breathing to calm your nervous system and strengthen your heart</p>
      </div>
      ...
      <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
        {TECHNIQUES.map((t) => (
          <button
            ...
            type="button"
            onClick={() => setTech(t)}
            className={`rounded-2xl border p-4 text-left transition ${tech.id === t.id ? "border-white/25 bg-white/10" : "border-[#e9f3f5]/10 bg-[#e9f3f5]/5 hover:bg-[#e9f3f5]/10"}`}
          ...
          >
            ...
            <p className="text-[11px] font-bold uppercase tracking-[0.14em] text-[#e9f3f5]/62">{t.tagline}</p>
            <p className="mt-2 text-xs leading-relaxed text-[#e9f3f5]/68">{t.benefit}</p>
          </button>
        ))}
      </div>
        ...
        <div className="relative flex flex-col items-center">
          ...
          <div className="relative grid h-72 w-72 place-items-center">
            ...
            <div
              ...
              style={{
                background: `radial-gradient(circle at 35% 30%, ${tech.accent}, #0e4a5a 70%)`,
          ...
          <div className="mt-6 flex items-center gap-3">
            <button
              ...
              style={{ background: `linear-gradient(90deg, ${tech.accent}, #0e7490)` }}
            >
            ...
            <button
              ...
              className="rounded-full border border-[#e9f3f5]/15 px-6 py-4 text-xs font-black uppercase tracking-[0.16em] text-[#e9f3f5]/70 hover:bg-[#e9f3f5]/8"
            >
              Reset
            </button>
          </div>
          <p className="mt-3 text-xs text-[#e9f3f5]/62">Follow the orb — breathe in as it grows, out as it shrinks. {tech.name} · {tech.tagline}</p>
        </div>
      </div>
      ...
      <div className="grid gap-4 sm:grid-cols-3">
        ...
        <div className="glass-card rounded-2xl p-6 text-center">
          ...
          <p className="mt-3 text-3xl font-black tabular-nums">{bpm}<span className="text-sm font-normal text-[#e9f3f5]/62"> bpm</span></p>
          <p className="text-[11px] text-[#e9f3f5]/62">{running ? "settling as you breathe…" : "start to see it drop"}</p>
        </div>
        ...
        <div className="glass-card rounded-2xl p-6 text-center">
          ...
          <div className="relative mx-auto mt-3 h-24 w-24">
            <div className="absolute inset-0 rounded-full" style={{ background: `conic-gradient(${cohColor} ${coherence * 3.6}deg, rgba(233,243,245,0.1) ${coherence * 3.6}deg)`, transition: "background 0.3s linear" }} />
            <div className="absolute inset-[7px] grid place-items-center rounded-full bg-[#0a141f]">
              <span className="text-2xl font-black tabular-nums" style={{ color: cohColor }}>{Math.round(coherence)}</span>
            </div>
          </div>
          <p className="mt-3 text-[11px] text-[#e9f3f5]/62">{coherence >= 75 ? "In the coherence zone! 🌊" : "keep the rhythm steady"}</p>
        </div>

        {/* Session */}
        <div className="glass-card rounded-2xl p-6 text-center">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-violet-300">Session</p>
          <p className="mt-3 text-4xl font-black tabular-nums">{mm}:{ss}</p>
          <p className="text-[11px] text-[#e9f3f5]/62">{cycles} full breaths</p>
          <div className="mt-3 h-2 overflow-hidden rounded-full bg-[#e9f3f5]/10">
            <div className="h-full rounded-full bg-gradient-to-r from-violet-400 to-fuchsia-400 transition-all duration-500" style={{ width: `${Math.min(100, (elapsed / 120) * 100)}%` }} />
          </div>
          <p className="mt-1.5 text-[10px] text-[#e9f3f5]/55">{elapsed >= 120 ? "Goal reached · +20 XP 🎉" : "Breathe 2 min for +20 XP"}</p>
        </div>
      </div>

      {/* Why it works */}
      <div className="glass-card rounded-2xl p-6">
        <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-violet-300">Why slow breathing helps your heart</p>
        <div className="grid gap-3 sm:grid-cols-3">
          {[
            ...
            <div key={s.t} className="rounded-xl border border-white/8 bg-white/5 p-4">
              <div className="text-2xl">{s.icon}</div>
              <p className="mt-2 text-sm font-bold text-[#e9f3f5]">{s.t}</p>
              <p className="mt-1 text-xs leading-relaxed text-[#e9f3f5]/68">{s.d}</p>
            </div>
          ))}
        </div>
        <p className="mt-4 text-[11px] text-[#e9f3f5]/55">Heart rate and coherence shown here are illustrative guides to keep you in rhythm, not medical measurements.</p>
      </div>
    </div>
  );
}
