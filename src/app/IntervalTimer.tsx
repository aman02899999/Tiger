  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/68">Tabata, EMOM, HIIT & custom rounds with audio cues</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2">
        {PRESETS.map((p) => (
          <button
            ...
            className={`rounded-full px-4 py-2 text-xs font-bold transition ${!useCustom && preset.id === p.id ? "bg-orange-500 text-white" : "border border-[#2a1e16]/12 bg-[#2a1e16]/5 text-[#2a1e16]/68 hover:text-[#2a1e16]"}`}
          >
            {p.name}
          </button>
        ))}
        <button
          ...
          className={`rounded-full px-4 py-2 text-xs font-bold transition ${useCustom ? "bg-orange-500 text-white" : "border border-[#2a1e16]/12 bg-[#2a1e16]/5 text-[#2a1e16]/68 hover:text-[#2a1e16]"}`}
        >
          ⚙️ Custom
        </button>
      </div>
        ...
        <div className="glass-card grid grid-cols-3 gap-3 rounded-2xl p-5">
          {([["work", "Work (s)"], ["rest", "Rest (s)"], ["rounds", "Rounds"]] as const).map(([k, label]) => (
            <label key={k} className="block">
              <span className="mb-1 block text-[11px] font-bold uppercase tracking-[0.12em] text-[#2a1e16]/65">{label}</span>
              <input
                type="number" min={1} value={custom[k]}
                onChange={(e) => setCustom({ ...custom, [k]: Math.max(1, Number(e.target.value)) })}
                className="w-full rounded-xl border border-[#2a1e16]/12 bg-[#fffdf9] px-3 py-2.5 text-sm outline-none focus:border-orange-200/40"
              />
            </label>
          ))}
        </div>
      ...
      <div className="glass-card rounded-3xl p-6 sm:p-10 text-center" style={{ background: `radial-gradient(ellipse at 50% 0%, ${meta.color}14 0%, transparent 60%)` }}>
        <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#2a1e16]/62">{active.name} · {active.tagline}</p>
        ...
        <div className="relative mx-auto mt-6 h-64 w-64">
          ...
          <div className="absolute inset-[12px] grid place-items-center rounded-full bg-[#fffdf9]">
            <div>
              ...
              {phase !== "idle" && phase !== "done" && (
                <p className="mt-1 text-xs font-bold text-[#2a1e16]/62">Round {round}/{active.rounds}{active.sets > 1 ? ` · Set ${setNum}/${active.sets}` : ""}</p>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="mt-8 flex items-center justify-center gap-3">
          <button
            ...
            style={{ background: `linear-gradient(90deg, ${meta.color}, #c2410c)` }}
          >
            {running ? "⏸ Pause" : phase === "done" ? "↻ Again" : phase === "idle" ? "▶ Start" : "▶ Resume"}
          </button>
          <button type="button" onClick={reset} className="rounded-full border border-[#2a1e16]/15 px-6 py-4 text-xs font-black uppercase tracking-[0.16em] text-[#2a1e16]/70 hover:bg-[#2a1e16]/8">Reset</button>
        </div>

        {phase === "done" && <p className="mt-4 text-sm font-bold text-emerald-600">Session complete! +25 XP earned 🎉</p>}
      </div>
      ...
      <div className="grid grid-cols-3 gap-3">
        {[
          ...
          <div key={s.label} className="glass-card rounded-2xl p-4 text-center">
            <p className="text-xl font-black tabular-nums text-[#ea580c]">{s.value}</p>
            <p className="text-[10px] font-bold uppercase tracking-[0.12em] text-[#2a1e16]/62">{s.label}</p>
          </div>
        ))}
      </div>

      <p className="text-center text-[11px] text-[#2a1e16]/55">🔊 Turn your volume up — the timer beeps a 3-second countdown and a distinct tone on every work/rest change.</p>
    </div>
  );
}
