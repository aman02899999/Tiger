    ...
    <div className="space-y-6">
      <div className="flex flex-wrap items-end justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-[#e9f3f5]/50">Personalized Indian meal plans based on your goals</p>
        </div>
        <div className="flex gap-2">
          <select value={goal} onChange={(e) => setGoal(e.target.value as any)} className="rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-2.5 text-sm text-[#e9f3f5] outline-none focus:border-violet-200/40">
          ...
          <select value={dietType} onChange={(e) => setDietType(e.target.value as any)} className="rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-2.5 text-sm text-[#e9f3f5] outline-none focus:border-violet-200/40">
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-5">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#e9f3f5]/44">Daily Calories</p>
          ...
          <p className="text-xs text-[#e9f3f5]/50">kcal/day</p>
        </div>
        <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-5">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#e9f3f5]/44">Protein</p>
          ...
          <p className="text-xs text-[#e9f3f5]/50">{calculations.proteinCal} kcal ({Math.round((calculations.proteinCal / calculations.targetCalories) * 100)}%)</p>
        </div>
        <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-5">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#e9f3f5]/44">Carbs</p>
          <p className="mt-3 bg-gradient-to-r from-[#ffb627] to-orange-400 bg-clip-text text-4xl font-black text-transparent">{calculations.carbs}g</p>
          <p className="text-xs text-[#e9f3f5]/50">{calculations.carbsCal} kcal ({Math.round((calculations.carbsCal / calculations.targetCalories) * 100)}%)</p>
        </div>
        <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-5">
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#e9f3f5]/44">Fat</p>
          ...
          <p className="text-xs text-[#e9f3f5]/50">{calculations.fatCal} kcal ({Math.round((calculations.fatCal / calculations.targetCalories) * 100)}%)</p>
        </div>
      </div>
        ...
        <div className="space-y-3">
          <div>
            <div className="mb-2 flex justify-between text-sm">
              ...
              <span className="text-[#e9f3f5]/60">{Math.round((calculations.proteinCal / calculations.targetCalories) * 100)}%</span>
            </div>
          ...
          <div>
            <div className="mb-2 flex justify-between text-sm">
              <span className="font-semibold text-[#ffb627]">Carbs</span>
              <span className="text-[#e9f3f5]/60">{Math.round((calculations.carbsCal / calculations.targetCalories) * 100)}%</span>
            </div>
            <div className="h-3 overflow-hidden rounded-full bg-[#e9f3f5]/10">
              <div className="h-full rounded-full bg-gradient-to-r from-[#ffb627] to-orange-400" style={{ width: `${(calculations.carbsCal / calculations.targetCalories) * 100}%` }} />
            </div>
          </div>
          <div>
            <div className="mb-2 flex justify-between text-sm">
              <span className="font-semibold text-rose-300">Fat</span>
              <span className="text-[#e9f3f5]/60">{Math.round((calculations.fatCal / calculations.targetCalories) * 100)}%</span>
            </div>
            <div className="h-3 overflow-hidden rounded-full bg-[#e9f3f5]/10">
      ...
      <div className="space-y-4">
        ...
        {allMeals.map((mealData, i) => (
          <div key={i} className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6">
            ...
            <div className="grid gap-3 md:grid-cols-3">
              {mealData.items.map((item: any, j: number) => (
                <div key={j} className={`rounded-xl border p-4 transition hover:bg-[#e9f3f5]/5 ${j === 0 ? "border-violet-200/30 bg-violet-200/10" : "border-[#e9f3f5]/8"}`}>
                  ...
                  <p className="text-sm font-semibold text-[#e9f3f5]">{item.name}</p>
                  <div className="mt-3 space-y-1 text-xs text-[#e9f3f5]/50">
                    <p>🔥 {item.kcal} kcal</p>
      ...
      <div className="rounded-2xl border border-[#ffb627]/20 bg-gradient-to-br from-[#ffb627]/10 to-violet-200/8 p-6">
        ...
        <div className="mt-4 grid gap-4 sm:grid-cols-4">
          <div>
            <p className="text-xs text-[#e9f3f5]/50">Calories</p>
            <p className="text-2xl font-black text-[#e9f3f5]">{dailyTotals.kcal}</p>
          </div>
          <div>
            <p className="text-xs text-[#e9f3f5]/50">Protein</p>
            <p className="text-2xl font-black text-emerald-300">{dailyTotals.protein}g</p>
          </div>
          <div>
            <p className="text-xs text-[#e9f3f5]/50">Carbs</p>
            <p className="text-2xl font-black text-[#ffb627]">{dailyTotals.carbs}g</p>
          </div>
          <div>
            <p className="text-xs text-[#e9f3f5]/50">Fat</p>
            <p className="text-2xl font-black text-rose-300">{dailyTotals.fat}g</p>
          </div>
        </div>
        <p className="mt-4 text-sm text-[#e9f3f5]/60">
      ...
      <div className="rounded-2xl border border-violet-200/20 bg-violet-200/8 p-6">
        ...
        <ul className="space-y-2.5 text-sm text-[#e9f3f5]/68">
