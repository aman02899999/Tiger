...
const SEGMENTS = [
  { label: "20 XP", xp: 20, color: "#0e7490" },
  { label: "50 XP", xp: 50, color: "#2dd4bf" },
  { label: "10 XP", xp: 10, color: "#3b9dff" },
  { label: "100 XP", xp: 100, color: "#ffb627" },
  { label: "30 XP", xp: 30, color: "#14b8a6" },
  { label: "75 XP", xp: 75, color: "#5eead4" },
  { label: "15 XP", xp: 15, color: "#93d0fd" },
  { label: "200 XP", xp: 200, color: "#ffb627" },
];
...
function SpinWheel({ email }: { email: string }) {
  ...
  return (
    <div className="glass-card rounded-3xl p-6 sm:p-8 text-center">
      <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#ffb627]">🎡 Daily Spin</p>
      <h2 className="mt-2 text-2xl font-black">Spin for Bonus XP</h2>
      <p className="mt-1 text-sm text-[#e9f3f5]/68">One free spin every day — up to 200 XP</p>
      ...
      <div className="relative mx-auto mt-8 h-72 w-72">
        ...
        {/* wheel */}
        <div
          className="h-full w-full rounded-full border-4 border-[#ffb627]/50 shadow-[0_0_60px_rgba(255,182,39,0.25),inset_0_0_40px_rgba(0,0,0,0.4)]"
          style={{
        ...
        {/* hub */}
        <div className="absolute left-1/2 top-1/2 grid h-16 w-16 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-2 border-[#ffb627]/60 bg-[#0a141f] text-2xl shadow-[0_0_24px_rgba(255,182,39,0.4)]">
          ⚡
        </div>
      </div>

      {won !== null && (
        <div className="mx-auto mt-6 inline-flex items-center gap-2 rounded-full border border-[#ffb627]/40 bg-[#ffb627]/12 px-6 py-3">
          <span className="text-xl">🎉</span>
          <span className="font-black text-[#ffb627]">You won +{won} XP!</span>
        </div>
      )}
      ...
      <div className="mt-6">
        <button
          ...
          className={`btn-gloss rounded-full px-12 py-4 text-xs font-black uppercase tracking-[0.2em] transition ${
            spunToday
              ? "cursor-not-allowed border border-[#e9f3f5]/15 bg-[#e9f3f5]/6 text-[#e9f3f5]/60"
              : "bg-gradient-to-r from-[#ffb627] to-orange-400 text-[#04121a] shadow-[0_12px_40px_rgba(255,182,39,0.3)]"
          }`}
        >
...
function WaterTracker({ email }: { email: string }) {
  ...
  return (
    <div className="glass-card rounded-3xl p-6 sm:p-8">
      ...
      <p className="mt-1 text-sm text-[#e9f3f5]/68">Hit {GOAL_ML / 1000}L today and earn +25 XP</p>
      ...
      <div className="mt-8 flex items-center gap-8">
        ...
        <div className="relative h-56 w-28 shrink-0 overflow-hidden rounded-b-3xl rounded-t-xl border-2 border-sky-300/30 bg-sky-300/5">
          <div
            ...
            style={{
              height: `${pct * 100}%`,
              background: "linear-gradient(180deg, rgba(147,208,253,0.85) 0%, rgba(96,182,250,0.9) 60%, rgba(34,128,240,0.95) 100%)",
              boxShadow: "0 -4px 20px rgba(96,182,250,0.4)",
            }}
          />
        ...
        <div className="min-w-0 flex-1">
          <p className="text-4xl font-black tabular-nums">
            {(ml / 1000).toFixed(2)}<span className="text-lg text-[#e9f3f5]/65"> / {GOAL_ML / 1000} L</span>
          </p>
          ...
          <div className="mt-5 flex flex-wrap gap-3">
            ...
            <button type="button" onClick={() => drink(-GLASS_ML)} className="rounded-full border border-[#e9f3f5]/15 px-4 py-3 text-xs font-bold text-[#e9f3f5]/65 hover:bg-[#e9f3f5]/8">
              Undo
            </button>
          </div>
        </div>
    ...
    <div className="space-y-8">
      <div className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-[#e9f3f5]/68">Free XP every day — spin, hydrate, stay consistent</p>
        </div>
        <div className="rounded-full border border-[#ffb627]/30 bg-[#ffb627]/10 px-5 py-2.5 text-sm font-black text-[#ffb627]">⚡ {xp.toLocaleString()} XP</div>
      </div>
