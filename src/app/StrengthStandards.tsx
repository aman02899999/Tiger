...
const TIERS = [
  { name: "Untrained", color: "#8fa8b8" },
  { name: "Novice", color: "#60b6fa" },
  { name: "Intermediate", color: "#34e08a" },
  { name: "Advanced", color: "#ffb627" },
  { name: "Elite", color: "#3b9dff" },
];
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#e9f3f5]/68">See how your lifts rank — from Untrained to Elite</p>
      </div>
      ...
      <div className="glass-card rounded-2xl p-6">
        <div className="flex flex-wrap items-end gap-4">
          <div>
            <p className="mb-2 text-xs font-bold uppercase tracking-[0.14em] text-[#e9f3f5]/65">Sex</p>
            <div className="inline-flex rounded-full border border-[#e9f3f5]/12 bg-[#e9f3f5]/5 p-1">
              {(["male", "female"] as Sex[]).map((s) => (
                <button key={s} type="button" onClick={() => setSex(s)} className={`rounded-full px-5 py-2 text-xs font-black uppercase tracking-[0.14em] transition ${sex === s ? "bg-violet-500 text-white" : "text-[#e9f3f5]/62"}`}>{s}</button>
              ))}
            </div>
          </div>
          <label className="block">
            <span className="mb-2 block text-xs font-bold uppercase tracking-[0.14em] text-[#e9f3f5]/65">Bodyweight (kg)</span>
            <input type="number" value={bw} onChange={(e) => setBw(Math.max(1, Number(e.target.value)))} className="w-32 rounded-xl border border-[#e9f3f5]/12 bg-[#0a141f] px-4 py-2.5 text-sm outline-none focus:border-violet-200/40" />
          </label>
        </div>
        ...
        <div className="mt-5 grid gap-3 sm:grid-cols-2">
          {LIFTS.map((l) => (
            <label key={l.key} className="block">
              <span className="mb-1 flex items-center gap-1.5 text-xs font-bold uppercase tracking-[0.12em] text-[#e9f3f5]/65"><span>{l.icon}</span>{l.label} 1RM (kg)</span>
              <input type="number" value={lifts[l.key] ?? ""} onChange={(e) => setLifts({ ...lifts, [l.key]: e.target.value })} placeholder="—" className="w-full rounded-xl border border-[#e9f3f5]/12 bg-[#0a141f] px-4 py-2.5 text-sm outline-none focus:border-violet-200/40" />
            </label>
          ))}
        </div>
      </div>
      ...
      {overall && (
        <div className="glass-card rounded-2xl p-6 text-center" style={{ background: `radial-gradient(ellipse at 50% 0%, ${overall.color}18 0%, transparent 60%)` }}>
          <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#e9f3f5]/62">Overall Rank</p>
            ...
            <div key={r.key} className="glass-card rounded-2xl p-5">
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <span className="ml-2 text-xs text-[#e9f3f5]/62">{r.ratio.toFixed(2)}× bw</span>
                </div>
              </div>
              {/* tier bar */}
              <div className="mt-3 flex gap-1">
                {TIERS.map((t, i) => (
                  <div key={t.name} className="h-2 flex-1 rounded-full" style={{ background: i < r.tier ? t.color : "rgba(233,243,245,0.08)" }} />
                ))}
              </div>
              <div className="mt-1.5 flex justify-between text-[9px] font-bold uppercase tracking-[0.08em] text-[#e9f3f5]/45">
      ...
      {results.every((r) => r.tier < 0) && (
        <div className="glass-card rounded-2xl p-8 text-center">
          ...
          <p className="mt-3 text-sm text-[#e9f3f5]/68">Enter at least one lift above to see your strength rank.</p>
        </div>
      )}

      <p className="text-center text-[11px] text-[#e9f3f5]/55">Standards are approximate bodyweight-ratio guidelines — great for direction, not gospel.</p>
    </div>
  );
}
