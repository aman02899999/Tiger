...
export default function CalorieGuessPage() {
  ...
  const grade = useMemo(() => {
    ...
    if (avg >= 55) return { label: "Sharp Eye 👀", color: "#a78bfa" };
    if (avg >= 30) return { label: "Getting Calibrated 📊", color: "#d8b35a" };
  ...
  if (done) {
    return (
      <div className="space-y-6">
        <div><h1 className="text-3xl font-black tracking-[-0.04em]">Your Calorie Intuition</h1><p className="text-sm text-[#f7f0df]/68">How close were your guesses?</p></div>
        <div className="glass-card rounded-3xl p-8 text-center" style={{ background: `radial-gradient(ellipse at 50% 0%, ${grade.color}22 0%, transparent 60%)` }}>
          ...
          <p className="text-[11px] uppercase tracking-[0.16em] text-[#f7f0df]/55">points of {ROUNDS * 100}</p>
        ...
        <p className="text-center text-[11px] text-[#f7f0df]/55">Building calorie awareness helps you eyeball portions without obsessive tracking.</p>
      </div>
    );
  }
  ...
  return (
    <div className="space-y-6">
      <div><h1 className="text-3xl font-black tracking-[-0.04em]">Calorie Guess Game</h1><p className="text-sm text-[#f7f0df]/68">Guess the calories to sharpen your food intuition</p></div>
      <div className="flex items-center justify-between text-xs font-bold text-[#f7f0df]/60"><span>Round {round + 1} of {ROUNDS}</span><span className="text-[#d8b35a]">{points} pts</span></div>

      <div className="glass-card rounded-3xl p-8 text-center">
        <div className="text-6xl">{food.icon}</div>
        <p className="mt-3 text-2xl font-black">{food.name}</p>
        <p className="text-xs text-[#f7f0df]/55">{food.portion}</p>

        <div className="mx-auto mt-6 max-w-md">
          <p className="mb-2 text-4xl font-black tabular-nums text-violet-200">{guess}<span className="text-lg text-[#f7f0df]/50"> kcal</span></p>
          <input type="range" min={20} max={500} step={5} value={guess} onChange={(e) => setGuess(Number(e.target.value))} disabled={locked} className="w-full accent-violet-400 disabled:opacity-50" />
          <div className="flex justify-between text-[10px] text-[#f7f0df]/45"><span>20</span><span>500</span></div>
        </div>

        {locked ? (
          <div className="mx-auto mt-5 max-w-md">
            <div className="rounded-2xl border border-[#f7f0df]/10 bg-[#f7f0df]/5 p-5">
              <p className="text-sm text-[#f7f0df]/68">Actual: <span className="text-2xl font-black text-[#d8b35a]">{food.kcal} kcal</span></p>
              <p className="mt-1 text-sm font-bold" style={{ color: pct <= 10 ? "#34d399" : pct <= 30 ? "#d8b35a" : "#fb7185" }}>{pct <= 10 ? "🎯 Spot on!" : pct <= 30 ? "👍 Close!" : "📊 Off by a bit"} — {err} kcal away (+{roundPts} pts)</p>
            </div>
            <button type="button" onClick={next} className="btn-gloss mt-4 w-full rounded-full bg-gradient-to-r from-violet-300 via-fuchsia-500 to-violet-700 py-3 text-xs font-black uppercase tracking-[0.16em] text-white">{round < ROUNDS - 1 ? "Next Food →" : "See Score"}</button>
          </div>
        ) : (
