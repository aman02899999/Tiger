  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/68">Estimate your body-fat % with the U.S. Navy tape method — just a measuring tape needed</p>
      </div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            ...
            <label key={f.label} className="block">
              <span className="mb-1 block text-[11px] font-bold uppercase tracking-[0.12em] text-[#2a1e16]/65">{f.label}{f.hint && <span className="ml-1 font-normal text-[#2a1e16]/45 normal-case tracking-normal">· {f.hint}</span>}</span>
              <input type="number" inputMode="decimal" value={f.v} onChange={(e) => f.set(e.target.value)} className="w-full rounded-xl border border-[#2a1e16]/12 bg-[#fffdf9] px-4 py-2.5 text-sm outline-none focus:border-orange-200/40" />
            </label>
          ))}
        </div>
      </div>
      ...
      {result ? (
            ...
            <div className="relative mx-auto h-40 w-40">
              ...
              <div className="absolute inset-[10px] grid place-items-center rounded-full bg-[#fffdf9]">
                <div>
                  ...
                  <p className="text-[10px] uppercase tracking-[0.16em] text-[#2a1e16]/62">body fat</p>
                </div>
              </div>
            </div>
          ...
          {result.fatMass > 0 && (
            <div className="grid grid-cols-2 gap-3">
              <div className="glass-card rounded-2xl p-5 text-center">
                <p className="text-2xl font-black tabular-nums text-[#ea580c]">{result.leanMass.toFixed(1)} kg</p>
                <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#2a1e16]/62">Lean mass</p>
              </div>
              <div className="glass-card rounded-2xl p-5 text-center">
                <p className="text-2xl font-black tabular-nums text-amber-600">{result.fatMass.toFixed(1)} kg</p>
                <p className="text-[11px] font-bold uppercase tracking-[0.12em] text-[#2a1e16]/62">Fat mass</p>
              </div>
            </div>
          )}
            ...
            <div className="flex gap-1">
              {CATEGORIES[sex].map((c) => (
                <div key={c.label} className="flex-1 text-center">
                  ...
                  <p className="mt-1.5 text-[9px] font-bold uppercase tracking-[0.06em] text-[#2a1e16]/55">{c.label}</p>
                </div>
              ))}
            </div>
          </div>
        ...
        <div className="glass-card rounded-2xl p-8 text-center">
          ...
          <p className="mt-3 text-sm text-[#2a1e16]/68">Fill in your height, neck, and waist{sex === "female" ? " and hip" : ""} measurements to see your estimate.</p>
        </div>
      )}

      <p className="text-center text-[11px] text-[#2a1e16]/55">The tape method is an estimate (±3–4%). For accuracy use a DEXA scan — but this is great for tracking change over time.</p>
    </div>
  );
}
