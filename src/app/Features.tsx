  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/50">Step-by-step guide to achieve your {user.goal.replace("-", " ")} goal</p>
      </div>

      <div className="space-y-6">
        {steps.map((phase: any, i: number) => (
          <div key={i} className="relative rounded-2xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-6">
            <div className="absolute -left-3 top-6 grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-orange-300 via-amber-500 to-[#ea580c] text-xs font-black text-[#f4ead9]">{i + 1}</div>
            <div className="ml-6">
              <div className="flex items-baseline gap-3">
                <span className="rounded-full bg-orange-200/15 px-3 py-1 text-xs font-bold uppercase tracking-[0.16em] text-orange-700">{phase.week}</span>
              ...
              <ul className="mt-4 space-y-2.5">
                {phase.tasks.map((task: string, j: number) => (
                  <li key={j} className="flex items-start gap-3">
                    ...
                    <span className="text-sm text-[#2a1e16]/72">{task}</span>
                  </li>
                ))}
              </ul>
            </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#2a1e16]/50">Track your before & after progress</p>
        </div>
        <button type="button" onClick={() => setShowForm(!showForm)} className="rounded-full bg-gradient-to-r from-orange-300 via-amber-500 to-orange-700 px-6 py-3 text-xs font-black uppercase tracking-[0.18em] text-white">
      ...
      {showForm && (
        <div className="rounded-2xl border border-orange-200/20 bg-orange-200/8 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-orange-700/70">Title</span><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="mt-2 w-full rounded-xl border border-[#2a1e16]/12 bg-[#2a1e16]/6 px-4 py-3 text-sm outline-none" placeholder="My 3-Month Journey" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-orange-700/70">Before Date</span><input type="date" value={form.beforeDate} onChange={(e) => setForm({ ...form, beforeDate: e.target.value })} className="mt-2 w-full rounded-xl border border-[#2a1e16]/12 bg-[#2a1e16]/6 px-4 py-3 text-sm outline-none" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-orange-700/70">After Date</span><input type="date" value={form.afterDate} onChange={(e) => setForm({ ...form, afterDate: e.target.value })} className="mt-2 w-full rounded-xl border border-[#2a1e16]/12 bg-[#2a1e16]/6 px-4 py-3 text-sm outline-none" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-orange-700/70">Weight Before (kg)</span><input value={form.weightBefore} onChange={(e) => setForm({ ...form, weightBefore: e.target.value })} className="mt-2 w-full rounded-xl border border-[#2a1e16]/12 bg-[#2a1e16]/6 px-4 py-3 text-sm outline-none" placeholder="85" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-orange-700/70">Weight After (kg)</span><input value={form.weightAfter} onChange={(e) => setForm({ ...form, weightAfter: e.target.value })} className="mt-2 w-full rounded-xl border border-[#2a1e16]/12 bg-[#2a1e16]/6 px-4 py-3 text-sm outline-none" placeholder="75" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-orange-700/70">Notes</span><input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="mt-2 w-full rounded-xl border border-[#2a1e16]/12 bg-[#2a1e16]/6 px-4 py-3 text-sm outline-none" placeholder="Lost 10kg in 3 months!" /></label>
          </div>
          <button type="button" onClick={addTransformation} className="mt-6 rounded-full bg-gradient-to-r from-orange-300 via-amber-500 to-orange-700 px-7 py-3 text-xs font-black uppercase tracking-[0.2em] text-white">Save Transformation</button>
        </div>
      )}
        ...
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {transformations.map((t: any) => (
              <div key={t.id} className="rounded-2xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-6">
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {dummyTransformations.map((t, i) => (
            <div key={i} className="rounded-2xl border border-[#ea580c]/20 bg-gradient-to-br from-[#ea580c]/10 to-orange-200/8 p-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#ea580c]">Featured</p>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/50">Earn ₹100 for every friend who joins</p>
      </div>

      {/* Wallet Balance */}
      <div className="rounded-2xl border border-[#ea580c]/20 bg-gradient-to-br from-[#ea580c]/15 to-orange-200/8 p-8">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#ea580c]">Wallet Balance</p>
        <p className="mt-3 bg-gradient-to-r from-[#ea580c] to-orange-400 bg-clip-text text-6xl font-black text-transparent">₹{wallet.balance}</p>
        <p className="mt-2 text-sm text-[#2a1e16]/60">{wallet.balance < 500 ? `₹${500 - wallet.balance} more to redeem` : "Ready to redeem!"}</p>
        
        {wallet.balance >= 500 && (
          <div className="mt-6 space-y-3">
            <input type="text" value={upiId} onChange={(e) => setUpiId(e.target.value)} placeholder="Enter UPI ID (e.g., yourname@paytm)" className="w-full rounded-xl border border-[#2a1e16]/12 bg-[#2a1e16]/6 px-4 py-3 text-sm outline-none focus:border-orange-200/40" />
            <button type="button" onClick={handleRedeem} className="w-full rounded-full bg-gradient-to-r from-[#ea580c] to-orange-400 py-4 text-sm font-black uppercase tracking-[0.2em] text-[#f4ead9]">💸 Redeem to UPI</button>
          </div>
        )}
      </div>

      {/* Referral Code */}
      <div className="rounded-2xl border border-orange-200/20 bg-orange-200/10 p-6">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-orange-700">Your Referral Code</p>
        <div className="mt-3 flex items-center gap-3">
          <code className="flex-1 rounded-xl bg-[#fffdf9] px-4 py-3 text-2xl font-black text-orange-700">{referralCode}</code>
          <button type="button" onClick={() => { navigator.clipboard.writeText(referralCode); alert("Code copied!"); }} className="rounded-full bg-orange-200/20 px-5 py-3 text-xs font-bold text-orange-700">Copy</button>
        </div>
        <p className="mt-3 text-sm text-[#2a1e16]/60">Share this code with friends. When they sign up and purchase a plan, you earn ₹100!</p>
      </div>
      ...
      <div className="rounded-2xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-6">
        ...
        {wallet.transactions.length === 0 ? (
          <p className="text-sm text-[#2a1e16]/50">No transactions yet. Start referring friends!</p>
        ) : (
          <div className="space-y-3">
            {wallet.transactions.map((t: any, i: number) => (
              <div key={i} className="flex items-center justify-between rounded-xl border border-[#2a1e16]/8 bg-[#2a1e16]/5 p-4">
                <div>
      ...
      <div className="rounded-2xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-6">
        ...
        <div className="space-y-3 text-sm text-[#2a1e16]/68">
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/50">Top 3 win ₹15,000 supplement stack!</p>
      </div>
      ...
      {/* My Rank */}
      <div className="rounded-2xl border border-orange-200/30 bg-orange-200/10 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="grid h-14 w-14 place-items-center rounded-full bg-gradient-to-br from-orange-300 via-amber-500 to-[#ea580c] text-lg font-black text-[#f4ead9]">{user?.avatar}</div>
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-700">Your Rank</p>
              <p className="text-2xl font-black">#{myRank}</p>
            </div>
          </div>
          <div className="text-right">
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-orange-700">Your Score</p>
            <p className="text-3xl font-black text-orange-700">{myScore.toLocaleString()}</p>
          </div>
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-6">
        ...
        <div className="space-y-3">
          {dummyUsers.map((u, i) => (
            <div key={u.name} className="flex items-center justify-between rounded-xl border border-[#2a1e16]/8 bg-[#2a1e16]/5 p-4">
              <div className="flex items-center gap-4">
                <span className="text-2xl font-black text-[#2a1e16]/30">#{i + 1}</span>
                <div className="grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br from-orange-300 via-amber-500 to-[#ea580c] text-xs font-black text-[#f4ead9]">{u.avatar}</div>
                <div>
                  <p className="font-bold">{u.name}</p>
                  <p className="text-xs text-[#2a1e16]/50">🔥 {u.streak} day streak</p>
                </div>
              </div>
              <p className="text-lg font-black text-orange-700">{u.score.toLocaleString()}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      <div className="rounded-2xl border border-[#ea580c]/20 bg-gradient-to-br from-[#ea580c]/10 to-orange-200/8 p-6">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#ea580c]">🏆 Monthly Prize</p>
        <h3 className="mt-2 text-2xl font-black">₹15,000 Supplement Stack</h3>
        <p className="mt-2 text-sm text-[#2a1e16]/66">Top 3 users with highest Titan Score win premium supplement stacks worth ₹15,000. Titan Score includes daily activity, streak maintenance, transformation uploads, and goal completion.</p>
        <p className="mt-4 text-xs text-[#2a1e16]/40">Leaderboard resets on the 1st of every month. Winners announced on the 3rd.</p>
      </div>
    </div>
  );
}
