  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#e9f3f5]/50">Step-by-step guide to achieve your {user.goal.replace("-", " ")} goal</p>
      </div>

      <div className="space-y-6">
        {steps.map((phase: any, i: number) => (
          <div key={i} className="relative rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6">
            <div className="absolute -left-3 top-6 grid h-8 w-8 place-items-center rounded-full bg-gradient-to-br from-violet-300 via-fuchsia-500 to-[#ffb627] text-xs font-black text-[#04121a]">{i + 1}</div>
            <div className="ml-6">
              ...
              <ul className="mt-4 space-y-2.5">
                {phase.tasks.map((task: string, j: number) => (
                  <li key={j} className="flex items-start gap-3">
                    ...
                    <span className="text-sm text-[#e9f3f5]/72">{task}</span>
                  </li>
                ))}
              </ul>
            </div>
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-[#e9f3f5]/50">Track your before & after progress</p>
        </div>
      ...
      {showForm && (
        <div className="rounded-2xl border border-violet-200/20 bg-violet-200/8 p-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-violet-100/70">Title</span><input value={form.title} onChange={(e) => setForm({ ...form, title: e.target.value })} className="mt-2 w-full rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-3 text-sm outline-none" placeholder="My 3-Month Journey" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-violet-100/70">Before Date</span><input type="date" value={form.beforeDate} onChange={(e) => setForm({ ...form, beforeDate: e.target.value })} className="mt-2 w-full rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-3 text-sm outline-none" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-violet-100/70">After Date</span><input type="date" value={form.afterDate} onChange={(e) => setForm({ ...form, afterDate: e.target.value })} className="mt-2 w-full rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-3 text-sm outline-none" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-violet-100/70">Weight Before (kg)</span><input value={form.weightBefore} onChange={(e) => setForm({ ...form, weightBefore: e.target.value })} className="mt-2 w-full rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-3 text-sm outline-none" placeholder="85" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-violet-100/70">Weight After (kg)</span><input value={form.weightAfter} onChange={(e) => setForm({ ...form, weightAfter: e.target.value })} className="mt-2 w-full rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-3 text-sm outline-none" placeholder="75" /></label>
            <label className="block"><span className="text-xs font-bold uppercase tracking-[0.16em] text-violet-100/70">Notes</span><input value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} className="mt-2 w-full rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-3 text-sm outline-none" placeholder="Lost 10kg in 3 months!" /></label>
          </div>
        ...
        <div>
          ...
          <div className="grid gap-4 md:grid-cols-2">
            {transformations.map((t: any) => (
              <div key={t.id} className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6">
                <h3 className="text-lg font-bold">{t.title}</h3>
                <div className="mt-3 flex items-center gap-4">
                  <div className="text-center"><p className="text-xs text-[#e9f3f5]/50">Before</p><p className="text-2xl font-black">{t.weightBefore} kg</p></div>
                  <span className="text-2xl">→</span>
                  <div className="text-center"><p className="text-xs text-[#e9f3f5]/50">After</p><p className="text-2xl font-black text-emerald-300">{t.weightAfter} kg</p></div>
                </div>
                <p className="mt-3 text-sm text-[#e9f3f5]/60">{t.notes}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <div>
        ...
        <div className="grid gap-4 md:grid-cols-3">
          {dummyTransformations.map((t, i) => (
            <div key={i} className="rounded-2xl border border-[#ffb627]/20 bg-gradient-to-br from-[#ffb627]/10 to-violet-200/8 p-6">
              <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#ffb627]">Featured</p>
              <h3 className="mt-2 text-lg font-bold">{t.title}</h3>
              <p className="mt-1 text-sm text-[#e9f3f5]/60">by {t.user}</p>
              <div className="mt-4 flex items-center justify-between">
                <div><p className="text-xs text-[#e9f3f5]/50">Before</p><p className="text-xl font-black">{t.before}</p></div>
                <div><p className="text-xs text-[#e9f3f5]/50">After</p><p className="text-xl font-black text-emerald-300">{t.after}</p></div>
                <div><p className="text-xs text-[#e9f3f5]/50">Duration</p><p className="text-sm font-bold">{t.duration}</p></div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#e9f3f5]/50">Earn ₹100 for every friend who joins</p>
      </div>

      {/* Wallet Balance */}
      <div className="rounded-2xl border border-[#ffb627]/20 bg-gradient-to-br from-[#ffb627]/15 to-violet-200/8 p-8">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#ffb627]">Wallet Balance</p>
        <p className="mt-3 bg-gradient-to-r from-[#ffb627] to-orange-400 bg-clip-text text-6xl font-black text-transparent">₹{wallet.balance}</p>
        <p className="mt-2 text-sm text-[#e9f3f5]/60">{wallet.balance < 500 ? `₹${500 - wallet.balance} more to redeem` : "Ready to redeem!"}</p>
        
        {wallet.balance >= 500 && (
          <div className="mt-6 space-y-3">
            <input type="text" value={upiId} onChange={(e) => setUpiId(e.target.value)} placeholder="Enter UPI ID (e.g., yourname@paytm)" className="w-full rounded-xl border border-[#e9f3f5]/12 bg-[#e9f3f5]/6 px-4 py-3 text-sm outline-none focus:border-violet-200/40" />
            <button type="button" onClick={handleRedeem} className="w-full rounded-full bg-gradient-to-r from-[#ffb627] to-orange-400 py-4 text-sm font-black uppercase tracking-[0.2em] text-[#04121a]">💸 Redeem to UPI</button>
          </div>
        )}
      </div>
      ...
      <div className="rounded-2xl border border-violet-200/20 bg-violet-200/10 p-6">
        ...
        <div className="mt-3 flex items-center gap-3">
          <code className="flex-1 rounded-xl bg-[#0a141f] px-4 py-3 text-2xl font-black text-violet-100">{referralCode}</code>
        ...
        <p className="mt-3 text-sm text-[#e9f3f5]/60">Share this code with friends. When they sign up and purchase a plan, you earn ₹100!</p>
      </div>
      ...
      <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6">
        ...
        {wallet.transactions.length === 0 ? (
          <p className="text-sm text-[#e9f3f5]/50">No transactions yet. Start referring friends!</p>
        ) : (
          <div className="space-y-3">
            {wallet.transactions.map((t: any, i: number) => (
              <div key={i} className="flex items-center justify-between rounded-xl border border-[#e9f3f5]/8 bg-[#e9f3f5]/5 p-4">
                <div>
                  <p className="font-bold capitalize">{t.type === "referral" ? `Referral from ${t.from || "User"}` : "Redemption"}</p>
                  <p className="text-xs text-[#e9f3f5]/50">{t.date}</p>
                </div>
      ...
      <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6">
        ...
        <div className="space-y-3 text-sm text-[#e9f3f5]/68">
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#e9f3f5]/50">Top 3 win ₹15,000 supplement stack!</p>
      </div>

      {/* Top 3 */}
      <div className="grid gap-4 md:grid-cols-3">
        {dummyUsers.slice(0, 3).map((u, i) => (
          <div key={u.name} className={`relative rounded-2xl border p-6 ${i === 0 ? "border-[#ffb627]/40 bg-gradient-to-br from-[#ffb627]/15 to-violet-200/8" : "border-[#e9f3f5]/10 bg-[#e9f3f5]/5"}`}>
            {i === 0 && <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-[#ffb627] px-4 py-1 text-xs font-black text-[#04121a]">🏆 1st Place</div>}
            {i === 1 && <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-gray-300 px-4 py-1 text-xs font-black text-[#04121a]">🥈 2nd Place</div>}
            {i === 2 && <div className="absolute -top-3 left-1/2 -translate-x-1/2 rounded-full bg-orange-400 px-4 py-1 text-xs font-black text-[#04121a]">🥉 3rd Place</div>}
            <div className="mt-3 flex flex-col items-center text-center">
              <div className="grid h-20 w-20 place-items-center rounded-full bg-gradient-to-br from-violet-300 via-fuchsia-500 to-[#ffb627] text-2xl font-black text-[#04121a]">{u.avatar}</div>
              <p className="mt-4 text-lg font-bold">{u.name}</p>
              <p className="mt-2 bg-gradient-to-r from-violet-200 to-[#ffb627] bg-clip-text text-4xl font-black text-transparent">{u.score.toLocaleString()}</p>
              <p className="text-xs text-[#e9f3f5]/50">Titan Score</p>
      ...
      <div className="rounded-2xl border border-violet-200/30 bg-violet-200/10 p-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-4">
            <div className="grid h-14 w-14 place-items-center rounded-full bg-gradient-to-br from-violet-300 via-fuchsia-500 to-[#ffb627] text-lg font-black text-[#04121a]">{user?.avatar}</div>
            <div>
      ...
      <div className="rounded-2xl border border-[#e9f3f5]/10 bg-[#e9f3f5]/5 p-6">
        ...
        <div className="space-y-3">
          {dummyUsers.map((u, i) => (
            <div key={u.name} className="flex items-center justify-between rounded-xl border border-[#e9f3f5]/8 bg-[#e9f3f5]/5 p-4">
              <div className="flex items-center gap-4">
                <span className="text-2xl font-black text-[#e9f3f5]/30">#{i + 1}</span>
                <div className="grid h-10 w-10 place-items-center rounded-full bg-gradient-to-br from-violet-300 via-fuchsia-500 to-[#ffb627] text-xs font-black text-[#04121a]">{u.avatar}</div>
                <div>
                  <p className="font-bold">{u.name}</p>
                  <p className="text-xs text-[#e9f3f5]/50">🔥 {u.streak} day streak</p>
                </div>
              </div>
      ...
      <div className="rounded-2xl border border-[#ffb627]/20 bg-gradient-to-br from-[#ffb627]/10 to-violet-200/8 p-6">
        <p className="text-xs font-bold uppercase tracking-[0.28em] text-[#ffb627]">🏆 Monthly Prize</p>
        <h3 className="mt-2 text-2xl font-black">₹15,000 Supplement Stack</h3>
        <p className="mt-2 text-sm text-[#e9f3f5]/66">Top 3 users with highest Titan Score win premium supplement stacks worth ₹15,000. Titan Score includes daily activity, streak maintenance, transformation uploads, and goal completion.</p>
        <p className="mt-4 text-xs text-[#e9f3f5]/40">Leaderboard resets on the 1st of every month. Winners announced on the 3rd.</p>
      </div>
    </div>
  );
}
