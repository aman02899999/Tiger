  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/68">Tap any muscle on the body — or in the list — to learn what it does</p>
      </div>
      ...
      <div className="flex justify-center">
        <div className="inline-flex rounded-full border border-[#2a1e16]/12 bg-[#2a1e16]/5 p-1">
          {(["front", "back"] as View[]).map((v) => (
            <button key={v} type="button" onClick={() => { setView(v); setSelected(null); }} className={`rounded-full px-6 py-2 text-xs font-black uppercase tracking-[0.14em] transition ${view === v ? "bg-orange-500 text-white" : "text-[#2a1e16]/62"}`}>
              {v === "front" ? "Front" : "Back"}
            </button>
          ))}
        </div>
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        ...
        <div className="glass-card rounded-3xl p-6">
          <div className="relative mx-auto w-full max-w-[300px]" style={{ aspectRatio: "3 / 5.2" }}>
            ...
            <div aria-hidden className="absolute left-1/2 top-0 h-[9%] w-[15%] -translate-x-1/2 rounded-full bg-[#2a1e16]/8" />
            <div aria-hidden className="absolute left-1/2 top-[10%] h-[45%] w-[52%] -translate-x-1/2 rounded-[40%_40%_30%_30%] bg-[#2a1e16]/5" />
            <div aria-hidden className="absolute left-1/2 top-[52%] h-[46%] w-[46%] -translate-x-1/2 rounded-[30%_30%_40%_40%] bg-[#2a1e16]/4" />
            ...
            {zones.map((m) => (
              <button
                ...
                style={{
                  ...
                  background: isSelected(m) ? "linear-gradient(135deg, rgba(249,115,22,0.6), rgba(251,146,60,0.5))" : "rgba(249,115,22,0.16)",
                  borderColor: isSelected(m) ? "rgba(251,146,60,0.8)" : "rgba(249,115,22,0.3)",
                  boxShadow: isSelected(m) ? "0 0 20px rgba(249,115,22,0.5)" : "none",
                }}
              />
            ))}
          </div>
          <p className="mt-3 text-center text-[11px] text-[#2a1e16]/50">{view === "front" ? "Anterior view" : "Posterior view"} · highlighted = selected</p>
        </div>
        ...
        <div className="space-y-3">
          {active ? (
            <div className="glass-card rounded-2xl p-6">
              <div className="flex items-start justify-between">
                <div>
                  ...
                  <p className="text-xs italic text-[#2a1e16]/60">{active.latin} · {active.region}</p>
                </div>
                <button type="button" onClick={() => setSelected(null)} className="rounded-full border border-[#2a1e16]/15 px-3 py-1.5 text-xs text-[#2a1e16]/70 hover:bg-[#2a1e16]/8">✕</button>
              </div>
              <p className="mt-4 text-sm leading-relaxed text-[#2a1e16]/82">{active.fn}</p>
              <div className="mt-4 rounded-xl border border-[#ea580c]/20 bg-[#ea580c]/8 p-3">
                <p className="text-xs leading-relaxed text-[#2a1e16]/78">💡 {active.tip}</p>
              </div>
              <div className="mt-4">
                <p className="mb-2 text-xs font-bold uppercase tracking-[0.16em] text-orange-600">Best exercises</p>
                <div className="flex flex-wrap gap-1.5">
                  {active.exercises.map((ex) => (
                    <span key={ex} className="rounded-full border border-orange-200/25 bg-orange-200/8 px-3 py-1.5 text-[11px] font-bold text-orange-700">{ex}</span>
                  ))}
                </div>
              </div>
            </div>
            ...
            <div className="glass-card rounded-2xl p-5">
              <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-orange-600">{view === "front" ? "Front" : "Back"} muscles ({listItems.length})</p>
              <div className="space-y-1.5">
                {listItems.map((m) => (
                  <button key={m.id} type="button" onClick={() => setSelected(m.id)} className="flex w-full items-center gap-3 rounded-xl border border-[#2a1e16]/10 bg-[#2a1e16]/5 p-3 text-left transition hover:bg-[#2a1e16]/10">
                    <span className="text-xl">{m.emoji}</span>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-semibold">{m.name}</p>
                      <p className="text-[11px] italic text-[#2a1e16]/55">{m.latin}</p>
                    </div>
                    <span className="text-[#2a1e16]/40">›</span>
                  </button>
                ))}
              </div>
            </div>
