  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[#e9f3f5]/68">Earn XP from workouts, level up, unlock badges</p>
      </div>
      ...
      <div className="glass-card relative overflow-hidden rounded-3xl p-6 sm:p-8">
        <div
          ...
          style={{ background: "radial-gradient(circle, rgba(255,182,39,0.6) 0%, transparent 70%)" }}
        />
        <div className="flex flex-wrap items-center gap-6">
          <div className="grid h-24 w-24 place-items-center rounded-3xl bg-gradient-to-br from-violet-300/25 to-[#ffb627]/20 text-6xl shadow-[0_0_40px_rgba(255,182,39,0.2)]">
            {level.icon}
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#ffb627]">Level {levelIdx + 1}</p>
            <h2 className="mt-1 text-3xl font-black tracking-[-0.02em]">{level.name}</h2>
            <p className="mt-1 text-sm text-[#e9f3f5]/68">
              {xp.toLocaleString()} XP{next ? ` · ${(next.min - xp).toLocaleString()} XP to ${next.name} ${next.icon}` : " · Max level reached!"}
            </p>
          </div>
          <div className="text-right">
            <p className="text-4xl font-black text-[#ffb627]">{unlockedCount}<span className="text-xl text-[#e9f3f5]/60">/{BADGES.length}</span></p>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#e9f3f5]/65">Badges</p>
          </div>
        </div>
        <div className="mt-6 h-3 overflow-hidden rounded-full bg-[#e9f3f5]/10">
          <div
            className="h-full rounded-full bg-gradient-to-r from-violet-300 via-fuchsia-400 to-[#ffb627] transition-all duration-1000"
            style={{ width: `${Math.min(levelProgress * 100, 100)}%` }}
          />
        </div>
        <div className="mt-3 flex justify-between text-[10px] font-bold uppercase tracking-[0.16em] text-[#e9f3f5]/62">
        ...
        {LEVELS.map((l, i) => (
          <div
            ...
            className={`flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold ${
              i <= levelIdx
                ? "border border-[#ffb627]/40 bg-[#ffb627]/12 text-[#ffb627]"
                : "border border-[#e9f3f5]/10 bg-[#e9f3f5]/4 text-[#e9f3f5]/62"
            }`}
          >
            <span>{l.icon}</span>
            <span>{l.name}</span>
          ...
          return (
            <div
              ...
              className={`rounded-2xl p-5 transition-all ${
                  ...
                  : "border border-[#e9f3f5]/8 bg-[#e9f3f5]/3 opacity-75"
              }`}
            >
              <div className="flex items-start gap-4">
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <p className="mt-0.5 text-xs text-[#e9f3f5]/68">{b.desc}</p>
                </div>
              ...
              {!done && (
                <div className="mt-4">
                  <div className="h-1.5 overflow-hidden rounded-full bg-[#e9f3f5]/10">
                  ...
                  <p className="mt-1.5 text-right text-[10px] font-bold text-[#e9f3f5]/62">{Math.min(cur, target).toLocaleString()} / {target.toLocaleString()}</p>
                </div>
              )}
            </div>
          );
