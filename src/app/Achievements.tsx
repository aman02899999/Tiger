  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="text-sm text-[#2a1e16]/68">Earn XP from workouts, level up, unlock badges</p>
      </div>
      ...
      <div className="glass-card relative overflow-hidden rounded-3xl p-6 sm:p-8">
        ...
        <div className="flex flex-wrap items-center gap-6">
          <div className="grid h-24 w-24 place-items-center rounded-3xl bg-gradient-to-br from-orange-300/25 to-[#ea580c]/20 text-6xl shadow-[0_0_40px_rgba(234,88,12,0.2)]">
            {level.icon}
          </div>
          <div className="min-w-0 flex-1">
            <p className="text-xs font-bold uppercase tracking-[0.24em] text-[#ea580c]">Level {levelIdx + 1}</p>
            <h2 className="mt-1 text-3xl font-black tracking-[-0.02em]">{level.name}</h2>
            <p className="mt-1 text-sm text-[#2a1e16]/68">
              {xp.toLocaleString()} XP{next ? ` · ${(next.min - xp).toLocaleString()} XP to ${next.name} ${next.icon}` : " · Max level reached!"}
            </p>
          </div>
          <div className="text-right">
            <p className="text-4xl font-black text-[#ea580c]">{unlockedCount}<span className="text-xl text-[#2a1e16]/60">/{BADGES.length}</span></p>
            <p className="text-xs font-bold uppercase tracking-[0.2em] text-[#2a1e16]/65">Badges</p>
          </div>
        </div>
        <div className="mt-6 h-3 overflow-hidden rounded-full bg-[#2a1e16]/10">
          <div
            className="h-full rounded-full bg-gradient-to-r from-orange-300 via-amber-400 to-[#ea580c] transition-all duration-1000"
            style={{ width: `${Math.min(levelProgress * 100, 100)}%` }}
          />
        </div>
        <div className="mt-3 flex justify-between text-[10px] font-bold uppercase tracking-[0.16em] text-[#2a1e16]/62">
        ...
        {LEVELS.map((l, i) => (
          <div
            ...
            className={`flex items-center gap-2 rounded-full px-4 py-2 text-xs font-bold ${
              i <= levelIdx
                ? "border border-[#ea580c]/40 bg-[#ea580c]/12 text-[#ea580c]"
                : "border border-[#2a1e16]/10 bg-[#2a1e16]/4 text-[#2a1e16]/62"
            }`}
          >
            <span>{l.icon}</span>
            <span>{l.name}</span>
          ...
          return (
            <div
              ...
              className={`rounded-2xl p-5 transition-all ${
                  ...
                  : "border border-[#2a1e16]/8 bg-[#2a1e16]/3 opacity-75"
              }`}
            >
              ...
              {!done && (
                <div className="mt-4">
                  <div className="h-1.5 overflow-hidden rounded-full bg-[#2a1e16]/10">
                    <div className="h-full rounded-full bg-gradient-to-r from-orange-300 to-amber-400" style={{ width: `${Math.min((cur / target) * 100, 100)}%` }} />
                  </div>
                  <p className="mt-1.5 text-right text-[10px] font-bold text-[#2a1e16]/62">{Math.min(cur, target).toLocaleString()} / {target.toLocaleString()}</p>
                </div>
              )}
            </div>
          );
