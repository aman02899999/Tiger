  ...
  return (
    <div className="glass-card rounded-2xl p-4">
      <div className="mb-3 flex items-center justify-between">
        ...
        {shot && <span className="text-[11px] text-[#e9f3f5]/62">{shot.date}</span>}
      </div>

      <button
        type="button"
        onClick={() => inputRef.current?.click()}
        className="relative flex aspect-[3/4] w-full items-center justify-center overflow-hidden rounded-xl border-2 border-dashed border-[#e9f3f5]/15 bg-[#e9f3f5]/5 transition hover:border-violet-300/40"
      >
        {shot ? (
          <img src={shot.data} alt={`${slot} progress`} className="h-full w-full object-cover" />
        ) : (
          <span className="px-4 text-center text-xs text-[#e9f3f5]/62">{busy ? "Processing…" : `Tap to upload your ${slot} photo`}</span>
        )}
      </button>
      ...
      <div className="mt-3 flex items-center gap-2">
        <input
          ...
          className="w-full rounded-lg border border-[#e9f3f5]/12 bg-[#0a141f] px-3 py-2 text-sm outline-none focus:border-violet-200/40"
        />
      </div>
    </div>
  );
...
function CompareSlider({ before, after }: { before: Shot; after: Shot }) {
  ...
  return (
    <div className="glass-card rounded-2xl p-5">
      <p className="mb-3 text-xs font-bold uppercase tracking-[0.2em] text-[#ffb627]">Drag to reveal your transformation</p>
      <div
        ref={ref}
      ...
      >
        ...
        <div className="absolute inset-0 overflow-hidden" style={{ width: `${pos}%` }}>
          ...
          <span className="absolute left-2 top-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#e9f3f5]">Before</span>
        </div>
        <span className="absolute right-2 top-2 rounded-full bg-black/60 px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.12em] text-[#ffb627]">After</span>
        {/* handle */}
      ...
      {before.weight && after.weight && !Number.isNaN(wDiff) && (
        <div className="mt-4 text-center">
          <p className="text-sm text-[#e9f3f5]/75">
            Weight change: <span className={`font-black tabular-nums ${wDiff < 0 ? "text-emerald-300" : wDiff > 0 ? "text-[#ffb627]" : "text-[#e9f3f5]"}`}>{wDiff > 0 ? "+" : ""}{wDiff.toFixed(1)} kg</span>
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-[#e9f3f5]/68">Upload a before &amp; after and drag to see how far you've come</p>
      </div>
      ...
      {shots.before && shots.after ? (
        ...
        <div className="glass-card rounded-2xl p-8 text-center">
          ...
          <p className="mt-3 text-sm text-[#e9f3f5]/68">Upload both photos above to unlock the drag-to-compare slider (+20 XP).</p>
        </div>
      )}

      <p className="text-center text-xs text-[#e9f3f5]/55">🔒 Your photos never leave your device — they're stored only in this browser.</p>
    </div>
  );
}
