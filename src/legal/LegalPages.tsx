...
const legalContent: Record<LegalType, { title: string; updated: string; sections: Array<{ h: string; p: string[] }> }> = {
  ...
  privacy: {
    ...
    sections: [
      ...
      { h: "2. How We Use Your Data", p: ["To run the training, nutrition and progress features you or your gym use.", "To process subscription payments through the payment provider; card and UPI details are entered on the provider's own page and never reach Tiger.", "To send transactional messages your gym sends to you, and optional notifications you can switch off.", "Tiger does not train advertising models on your health records, and does not sell them."] },
      { h: "3. Data Sharing", p: ["We do not sell personal data.", "Your gym's owner and the trainer assigned to you can see the records needed to coach you — that is the point of the service, and it is stated here explicitly.", "Payment data is handled by the payment provider. Anonymous, aggregate product analytics may be collected if the operator enables them."] },
      { h: "4. Data Storage & Security", p: ["Data is stored in Google Cloud Firestore and Cloud Storage, which encrypt data at rest with Google-managed keys and in transit with TLS.", "The project region, retention settings and any customer-managed keys are chosen at deployment time by the operator of your Tiger instance — ask them where your data is stored.", "Access is governed by custom claims and per-tenant security rules: a member can read only their own records, a trainer only the clients assigned to them, and a gym owner only their own gym.", "We do not claim third-party penetration testing or a specific regulatory certification. The security architecture, including its invariants and the tests that assert them, is published in docs/SAAS_TRANSFORMATION.md."] },
  ...
  refund: {
    ...
    updated: "June 15, 2025",
    sections: [
      { h: "1. Free Trial", p: ["Where a trial is offered, its length and terms are shown on the checkout page before payment.", "A trial that is not backed by a verified payment-provider webhook is not offered at all — Tiger never simulates a subscription."] },
